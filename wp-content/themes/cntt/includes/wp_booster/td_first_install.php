...
//      die;
	...
	foreach ($instance->plugins as $plugin) {
		...
		if ( ( isset($plugin['td_install']) && $plugin['td_install'] ) || ( isset($plugin['td_install_if_exists']) && $plugin['td_install_if_exists'] ) ) {
			...
			$plugin_constant = str_replace('-', '_', strtoupper($plugin['slug']));
			$theme_plugin_version = '';
			if (isset($td_plugins[$plugin_constant])) {
				$theme_plugin_version = $td_plugins[$plugin_constant];
			}

			if (is_plugin_active($plugin_main)) {
				// plugin is active
				// there's a plugin version check in td_wp_booster_functions
				...
				//read plugin file
				$fp = fopen( $plugins_path . '/' . $plugin_main, 'r' );
				$file_data = fread( $fp, 8192 );
				fclose( $fp );

				preg_match('/define\(\'' . $plugin_constant . '\', \'(.*)\'\)/', $file_data, $matches);
				if (isset($matches[1]) && $matches[1] ===  $theme_plugin_version) {
					//plugin disabled by the user - don't activate it
					td_log::log(__FILE__, __FUNCTION__, 'Not Active: skipped update for plugin ' . $plugin['slug']);
					$skip_plugin_activation[] = $plugin['slug'];
					continue;
				}
			}
